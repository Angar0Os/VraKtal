...
void main()
{
    // Normalisation de la normale interpolée (elle peut avoir été dénormalisée par l'interpolation)
    vec3 normal = normalize(inNormal);
    
    // Éclairage directionnel amélioré
    vec3 lightDir = normalize(vec3(0.3f, 1.0f, 0.3f));
    float lightValue = max(dot(normal, lightDir), 0.1f);
    
    // Calcul de l'irradiance avec la normale normalisée
    vec3 irradiance = calcIrradiance(normal);
    
    // Échantillonnage de texture
    vec3 color = inColor * texture(colorTex, inUV).xyz;
    
    // Combinaison finale avec un facteur d'irradiance plus équilibré
    vec3 finalColor = color * lightValue + color * irradiance * 0.2f;
