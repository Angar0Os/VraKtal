...
namespace graphics
{
	...
	void GBufferPass::Init(Device& device)
	{
		if (m_debugEnabled)
	...
	void GBufferPass::UpdateDescriptorSets(uint32_t frameIndex)
	{
		(void)frameIndex;
	}

	void GBufferPass::BindDescriptorSets(CommandBuffer& cmd, uint32_t frameIndex)
	{
	...
	void GBufferPass::Draw(CommandBuffer& cmd,
	...
	void GBufferPass::SetMeshInstances(
		const std::vector<std::pair<resources::Mesh*, glm::mat4>>* instances)
	{
		m_meshInstances = instances;
	}

	const std::vector<PassAttachment>& GBufferPass::GetColorAttachments() const
	{
		return m_colorAttachments;
	}

	const PassAttachment* GBufferPass::GetDepthAttachment() const
	{
