...
core::Input::Input(core::Window& window, core::gpu::Device* _device) : m_internal(new Internal)
{
	...
	m_windowResize = [this](int width, int height)
		{
			if (width == 0 || height == 0)
				return; // Fenêtre minimisée, on ignore

			m_windowRef->SetSize(static_cast<uint32_t>(width), static_cast<uint32_t>(height));
			m_windowRef->framebufferResized = true;
		};
...
{
	if (key == -1) // fn sur certains claviers
		return;
...
void core::Input::mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (instance) {
		instance->m_mousePosition = glm::vec2(xpos, ypos);
	}
}
...
void core::Input::framebuffer_size_callback(GLFWwindow* window, int width, int height)
{
	...
	if (instance && instance->m_windowResize)
		instance->m_windowResize(width, height);
}
